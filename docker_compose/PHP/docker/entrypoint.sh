...
#  if [ ! -e "${php_software_logs}/xdebug/dbg" ];      then mkdir "${php_software_logs}/xdebug/dbg"; fi
  ...
  cp "${BASH_LOGIN}" "/home/${USER_PHP_NAME}/.bashrc"
  chown "${USER_PHP_NAME}:${GROUP_PHP_NAME}"    "/home/${USER_PHP_NAME}/.bashrc"
  chmod 700   "/home/${USER_PHP_NAME}/.bashrc"
  ...
  for variable_name in "${php_folders[@]}"; do
    chown -R "${USER_PHP_NAME}:${GROUP_PHP_NAME}"   "${variable_name}"
    chmod -R 750   "${variable_name}"
  done;

  chown -R "${USER_PHP_NAME}:${GROUP_PHP_NAME}"   "${PHP_SOFTWARE_HOME}"
  chmod -R 770   "${PHP_SOFTWARE_HOME}"
...
if [[ ( "${env_xdebug_on}" == "$YES" ) && ( "${marker_xdebug_installed}" != "$YES" ) ]]; then
  ...
  chown -R "${USER_PHP_NAME}:${GROUP_PHP_NAME}"   "/usr/local/bin/pie"
  chmod -R 750   "/usr/local/bin"
...
if [[ ( "${env_xdebug_on}" == "$YES" ) && ( "${marker_xdebug_installed}" == "$YES" ) ]]; then
  ...
  elif [[ ( "${env_xdebug_on}" != "$YES" ) && ( "${marker_xdebug_installed}" == "$YES" ) ]]; then
    if [[ "${WHETHER_DEV_MODE}" == "true" ]]; then
...
#  sudo -u $USER_PHP_NAME /bin/bash -c "echo ${USER_PHP_NAME}; nohup "/bin/bash -c ". /home/${USER_PHP_NAME}/.bashrc; php-fpm"" &"
  sudo -u $USER_PHP_NAME /bin/bash -c "echo ${USER_PHP_NAME}; . /home/${USER_PHP_NAME}/.bashrc; php-fpm" &

  if [[ "${WHETHER_DEV_MODE}" == "true" ]]; then
    echo -e "[$(date)]: After php-fpm started ...\n"
  fi
