...
# -- BASH LOGIN --
    ...
    if [[ ( "${marker_first_start}" == "$YES" ) && ( "${marker_bash_login_written}" != "$YES" ) ]]; then
      ...
      USER_A4DC_BASH_LOGIN="/home/${USER_A4DC_NAME}/.bashrc"
      ...
      if [ -e "${USER_A4DC_BASH_LOGIN}" ]; then
        rm     "${USER_A4DC_BASH_LOGIN}"
      fi
      ...
      echo    -e "${bash_login_content}" >> "${USER_A4DC_BASH_LOGIN}"
      cat        "${templates}/profile"  >> "${USER_A4DC_BASH_LOGIN}"
      ...
      chown -R "${USER_A4DC_NAME}:${GROUP_A4DC_NAME}"       "/home/${USER_A4DC_NAME}"
      ...
      chmod 700   "${USER_A4DC_BASH_LOGIN}"
